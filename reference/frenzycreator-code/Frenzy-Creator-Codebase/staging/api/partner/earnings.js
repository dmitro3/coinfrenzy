...
async function fetchAllNgrRows(supabase, affiliateEmail, players, usernameAliases) {
  ...
  // Build the OR filter parts. Any one of these must match for a row to be
  // pulled; we then re-filter in-process via rowBelongsToMe() to drop any
  // false positives (e.g. shared player_id across affiliates).
  //
  // Use .ilike (case-insensitive) for affiliate_email since CF may have
  // sent it in mixed case originally. Without this, rows whose
  // affiliate_email casing differs from our lowercased session email
  // would only be pulled via the player_id / player_email join — which
  // breaks if those joins are also missing.
  const orParts = ['affiliate_email.ilike.' + affiliateEmail];

  const playerIds = players.map(p => p.player_id || p.id).filter(Boolean);
  if (playerIds.length > 0) {
    // PostgREST .or only accepts string literals; quote ids defensively.
    orParts.push('player_id.in.(' + playerIds.map(id => '"' + String(id).replace(/"/g, '\\"') + '"').join(',') + ')');
  }

  const playerEmails = players.map(p => (p.player_email || '').toLowerCase()).filter(Boolean);
  if (playerEmails.length > 0) {
    orParts.push('player_email.in.(' + playerEmails.map(e => '"' + e.replace(/"/g, '\\"') + '"').join(',') + ')');
  }

  if (Array.isArray(usernameAliases) && usernameAliases.length > 0) {
    orParts.push('affiliate_username.in.(' + usernameAliases.map(u => '"' + u.replace(/"/g, '\\"') + '"').join(',') + ')');
  }

  const orExpr = orParts.join(',');

  const baseQuery = (offset) => supabase
    .from('ngr_data')
    .select(SELECT_COLS)
    .or(orExpr)
    .range(offset, offset + pageSize - 1);
  ...
  let offset = 0;
  while (true) {
    const { data, error } = await baseQuery(offset);
    if (error) {
      console.error('[partner/earnings] ngr_data page error:', error);
      break;
    }
    const rows = data || [];
    allRows.push(...rows);
