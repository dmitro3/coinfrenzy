...
export async function POST(req: NextRequest) {
  let parsed: z.output<typeof forgotBody>
  try {
    parsed = forgotBody.parse(await req.json())
  } catch (e) {
  ...
  if (users[0]) {
    ...
    if (!cooldown.allowed) {
      return NextResponse.json(
        { error: 'rate_limited', message: cooldown.message },
        { status: 401 },
      )
    }
    ...
    try {
      ...
      console.error('[forgot-password] sendPasswordResetOtpEmail failed', {
        email,
